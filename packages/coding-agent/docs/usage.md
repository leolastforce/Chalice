...
The generic Pi core does not include built-in MCP, sub-agents, permission popups, plan mode, to-dos, or background bash. Chalice bundles MCP support through its vendored `pi-mcp-adapter`; other workflows can be built or installed as extensions or packages, or provided by tools such as containers and tmux.
