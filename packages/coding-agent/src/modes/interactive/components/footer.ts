...
export const CHALICE_MODES: readonly ChaliceMode[] = [
  "Change",
  "Think",
  "Review",
];
...
export class FooterComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (totalNeeded <= width) {
      statsLine =
        statsLeft +
        " ".repeat(width - statsLeftWidth - identityWidth) +
        identity;
    } else {
      const availableForIdentity = width - statsLeftWidth - minPadding;
      if (availableForIdentity > 0) {
        const truncatedIdentity = truncateToWidth(
          identity,
          availableForIdentity,
          "",
        );
        const truncatedWidth = visibleWidth(truncatedIdentity);
        statsLine =
          statsLeft +
          " ".repeat(Math.max(0, width - statsLeftWidth - truncatedWidth)) +
          truncatedIdentity;
      } else {
        statsLine = statsLeft;
      }
