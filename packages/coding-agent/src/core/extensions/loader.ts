...
const isTypeScriptSourceRuntime =
  !isBunBinary && path.extname(fileURLToPath(import.meta.url)) === ".ts";
  ...
  const runtime: ExtensionRuntime = {
    ...
    unregisterProvider: (name) => {
      ...
      runtime.pendingNativeProviderRegistrations =
        runtime.pendingNativeProviderRegistrations.filter(
...
function createExtensionAPI(
  ...
  const api = {
    ...
    getFlag(name: string): boolean | string | undefined {
      ...
      return runtime.flagValues.has(name)
        ? runtime.flagValues.get(name)
        : pendingFlagValues.get(name);
    },
...
function isCurrentCacheToken(
  cacheToken: ExtensionCacheToken | undefined,
): cacheToken is ExtensionCacheToken {
  return (
