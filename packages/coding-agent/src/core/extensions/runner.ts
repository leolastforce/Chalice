...
type BuiltInKeyBindings = Partial<
  Record<KeyId, { keybinding: string; restrictOverride: boolean }>
>;
...
export class ExtensionRunner {
  ...
  bindCore(
    ...
    this.getSystemPromptOptionsFn =
      contextActions.getSystemPromptOptions ??
      (() => normalizeBuildSystemPromptOptions({ cwd: this.cwd }));

    // Flush provider registrations queued during extension loading
    for (const { name, config, extensionPath } of this.runtime
    ...
    for (const { provider, extensionPath } of this.runtime
  ...
  setUIContext(
    ...
    this.uiContext = uiContext
      ? this.wrapUIPromptContext(uiContext)
      : noOpUIContext;
    this.mode = mode;
  }
  ...
  getMarkdownTransformers(): MarkdownTransformer[] {
    return this.extensions.flatMap((ext) =>
      ext.markdownTransformer ? [ext.markdownTransformer] : [],
    );
  }
  ...
  getCommand(name: string): ResolvedCommand | undefined {
    return this.resolveRegisteredCommands().find(
      (command) => command.invocationName === name,
    );
  }

  /** Shared event bus for extension and host coordination. */
  get events(): EventBus {
    this.assertActive();
  ...
  async emitBeforeAgentStart(
    ...
    const currentOptions =
      normalizeBuildSystemPromptOptions(systemPromptOptions);
    const renderCurrentSystemPrompt = (): string =>
      buildSystemPrompt(currentOptions);
